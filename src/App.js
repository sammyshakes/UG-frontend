...
function App() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const _ownedForgeIds = await ugNftContract.getNftIDsForUser(
        accounts[0],
        4
      );
      const _balance = await bloodContract.balanceOf(accounts[0]);
      const ownedForgeIds = _ownedForgeIds?.map((id) => {
        return Number(id.toString());
      });
      ...
      const ownedIds = _ownedIds?.map((id) => {
        return Number(id.toString());
      });
      const _stakedFYs = await ugNftContract.getFighters(stakedIds);
      ...
      const ownedFYs = ownedIds?.map((id, i) => {
        ...
        imageUrl = baseUrl.concat(
          imageUrl.concat(_ownedFYs[i]?.imageId).concat(".png")
        );
  ...
  return (
    ...
    >
      <div
        className=" bg-img"
        style={{ height: { xs: "155vh", sm: "130vh", md: "100vh" } }}
      >
        <Stack spacing={6}>
          <Header />
          <Box sx={{ my: 0 }}></Box>
          ...
          >
            ...
            <Container>
              <Routes>
                <Route path="/">
                  <Route index element={<Market account={accounts} />} />
                  <Route
                    path="market/*"
                    element={<Market account={accounts} />}
                  />
