...
export default function FightClubMintBox() {
  ...
  const mintHandler = async (event) => {
    ...
    const walletFightClubs = await ugNftContract.getNftIDsForUser(
      accounts[0],
      5
    );
    ...
    console.log(
      "walletFightClubs",
      Number(walletFightClubs.length) + Number(enteredQty)
    );
    if (Number(walletFightClubs.length) + Number(enteredQty) > 5) {
      setError({
        title: "Slow Down Friend.. Only 5 Allowed",
        message:
          "There are other ways to gain wealth and prestige in the Underground",
      });
      return;
    }
    ...
    if (receipt.hash)
      setError({
        ...
        message:
          "Manage your Business Wisely...  Tx will complete momentarily..",
      });
  };
  ...
  return (
    ...
    >
      <div>
        ...
        <Stack spacing={5}>
          <Box
            className="fclub-bordr"
            sx={{ mb: 4, maxWidth: 500, maxHeight: 800 }}
          >
            <Card>
              <CardMedia component="img" image={imageUrl} alt="Fight Club" />
              <Typography
                variant="h6"
                align="center"
                sx={{
