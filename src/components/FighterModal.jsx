...
const FighterModal = (props) => {
  const [sizeQueues, setSizeQueues] = useState({
    size1: 0,
    size2: 0,
    size3: 0,
    size4: 0,
  ...
  const eligibleForWeapons = () => {
    if (
      (props.raider.level >= 10 &&
        (props.raider.knuckles === 0 || props.raider.knuckles % 5 === 1)) ||
      (props.raider.level >= 19 &&
        (props.raider.chains === 0 || props.raider.chains % 5 === 1)) ||
      (props.raider.level >= 28 &&
        (props.raider.butterfly === 0 || props.raider.butterfly % 5 === 1)) ||
      (props.raider.level >= 40 &&
        (props.raider.machete === 0 || props.raider.machete % 5 === 1)) ||
      (props.raider.level >= 52 &&
        (props.raider.katana === 0 || props.raider.katana % 5 === 1))
    ) {
  ...
  const exitRaidHandler = () => {
    props.hideModal();
    props.exitRaider(props.raider.id);
  };
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const size1Queue = await ugRaidContract.getRaiderQueueLength(
        levelTier,
        1
      );
      const size2Queue = await ugRaidContract.getRaiderQueueLength(
        levelTier,
        2
      );
      const size3Queue = await ugRaidContract.getRaiderQueueLength(
        levelTier,
        3
      );
      const size4Queue = await ugRaidContract.getRaiderQueueLength(
        levelTier,
        4
      );
      const isQueued = await ugRaidContract.viewIfRaiderIsInQueue(
        props.raider.id
      );
      setSizeQueues({
        size1: size1Queue,
              ...
              <Stack>
                <Stack direction="row" sx={{ justifyContent: "space-between" }}>
                  <Stack align="left" spacing={1} sx={{}}>
                    ...
                    >
                      Level: {props.raider.level}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1rem" }}
                    >
                      Brutality: {props.raider.brutality}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{
                    ...
                    >
                      Knucks: {getWeapon(props.raider.knuckles)}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{
                    ...
                    >
                      ButterFly: {getWeapon(props.raider.butterfly)}
                    </Typography>
                  </Stack>
                  <Stack align="center" spacing={1} sx={{}}>
                    ...
                    <Typography
                      ...
                      sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1rem" }}
                    >
                      Courage: {props.raider.courage}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{ fontFamily: "Alegreya Sans SC", fontSize: ".8rem" }}
                    >
                      Scars: {props.raider.scars}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{
                    ...
                    >
                      Machete: {getWeapon(props.raider.machete)}
                    </Typography>
                  </Stack>
                  <Stack align="right" spacing={1} sx={{}}>
                    ...
                    >
                      Chains: {getWeapon(props.raider.chains)}
                    </Typography>
                    <Typography
                      variant="body2"
                      sx={{
                    ...
                    >
                      Katana: {getWeapon(props.raider.katana)}
                    </Typography>
                  </Stack>
                </Stack>
                <Typography
                ...
                >
                  Last Raid Time:{" "}
                  {Math.floor(
                    (Date.now() / 1000 - props.raider.lastRaidTime) / 86400
                  )}{" "}
                  Days Ago{" "}
                </Typography>
              </Stack>
        ...
        <Box className="box-bordr">
          <Stack>
            <Typography
              variant="body2"
              align="center"
              sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.5rem" }}
            >
              Available Raids
            </Typography>
            <Stack direction="row" sx={{ justifyContent: "space-between" }}>
            ...
            <Stack direction="row" sx={{ justifyContent: "space-between" }}>
              ...
              <Stack sx={{}}>
                ...
                <Typography
                  ...
                  sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.5rem" }}
                >
                  {Math.floor(sizeQueues.size2 / 10)}
                </Typography>
                <Typography
                  variant="body2"
                  pl={3}
                  sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.5rem" }}
                >
                  {Math.floor(sizeQueues.size3 / 15)}
                </Typography>
                <Typography
                  variant="body2"
                  pl={3}
                  sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.5rem" }}
                >
                  {Math.floor(sizeQueues.size4 / 20)}
                </Typography>
              </Stack>
              <Stack sx={{}}>
                <Typography
                  variant="body2"
                  pl={4}
                  sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.4rem" }}
                >
                  {5 - (sizeQueues.size1 % 5)}
                </Typography>
                <Typography
                  variant="body2"
                  pl={4}
                ...
                <Typography
                  ...
                  sx={{ fontFamily: "Alegreya Sans SC", fontSize: "1.4rem" }}
                >
                  {20 - (sizeQueues.size4 % 20)}
                </Typography>
              </Stack>

              <Stack spacing={0.4} sx={{}}>
                {!isQueued && weaponEligible && (
                  <Typography
                    variant="body2"
                    py={0.5}
                    sx={{
            ...
            <Box className="sweat-box">
              <Stack direction="row" sx={{ justifyContent: "space-between" }}>
                ...
                >
                  <img src={sweatToken} alt="sweatToken" height="50/100" />{" "}
                </Box>
                <Box width={130} className="input-group m-1 sweat1-bordr">
                  <input
                    ...
                    min="100"
                    step="100"
                    onChange={SweatChangeHandler}
                    className="form-control"
                    placeholder={
                      remainingSweat < 0
                        ? "Available: " + sweatBalance.toString()
                        : "sweat: " + remainingSweat.toString()
                    }
                    aria-label="Qty"
