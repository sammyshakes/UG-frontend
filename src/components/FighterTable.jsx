      ...
      return (
        <Box className="progress-box" pt={1} px={1}>
          <CircularProgressWithLabel
            value={getRaidProgress(params.row.lastRaidTime)}
            time={getRaidTimeLeft(params.row.lastRaidTime)}
            style={{
              color:
                Math.round(getRaidProgress(params.row.lastRaidTime), 2) < 20
                  ? "red"
                  : "chartreuse",
            }}
          />
...
const IsInRaid = (props) => {
  ...
  return (
    <Box>
      {isRaiding && (
        <Box className="inraid-box">
          <img src={bloodIcon} alt="bloodIcon" height={40} />
        </Box>
      )}
...
export default function FighterTable() {
  ...
  const sortedRaidTimeList = filteredList?.sort(
  ...
  const eligibleForWeapons = (fighter) => {
    if (
      (fighter.level >= 10 &&
        (fighter.knuckles === 0 || fighter.knuckles % 5 === 1)) ||
      (fighter.level >= 19 &&
        (fighter.chains === 0 || fighter.chains % 5 === 1)) ||
      (fighter.level >= 28 &&
        (fighter.butterfly === 0 || fighter.butterfly % 5 === 1)) ||
      (fighter.level >= 40 &&
        (fighter.machete === 0 || fighter.machete % 5 === 1)) ||
      (fighter.level >= 52 &&
        (fighter.katana === 0 || fighter.katana % 5 === 1))
    ) {
      return false;
    }
    return true;
  ...
  const updateRaidRewards = async () => {
    ...
    const userRaidRewards = await ugRaidContract.raiderOwnerBloodRewards(
      accounts[0]
    );
  ...
  return (
    <div>
      ...
      {refereeIsShown && <RefereeModal hideModal={hideRefereeHandler} />}
      ...
      >
        ...
        >
          <Alert
            ...
            sx={{ width: "100%" }}
          >
            Patience.. this tx could take a while... Unequipped Fighters will be
            sent home...
          </Alert>
        </Snackbar>
        <Box
        ...
        <Stack height="80vh">
          ...
          <Box className="box20-bordr" height={"80vh"}>
            {raidTickets?.length > 0 && (
              <ImageList
                sx={{ maxHeight: 9 / 10, width: 290 }}
                cols={3}
                rowHeight={150}
              >
              ...
              >
                <Box width={130} className="input-group m-1 ">
                  <input
                    ...
                    min="10"
                    step="10"
                    onChange={enteredBulkRaidersChangeHandler}
