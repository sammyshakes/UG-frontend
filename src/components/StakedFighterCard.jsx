...
const StakedFighterCard = (props) => {
  ...
  const [bloodCostToRank, setBloodCostToRank] = useState();
  ...
  const getUpdates = async () => {
    ...
    const stakedAmuletId = await ugArenaContract.getStakedAmuletIDForUser(
      accounts[0]
    );
    const amulet = await ugNftContract.getRingAmulet(Number(stakedAmuletId));
    const fighterUnclaimed = await ugArenaContract.calculateStakingRewards(
      props.id
    );
    const yakuzaUnclaimed = await ugYakDenContract.calculateStakingRewards(
      props.id
    );
    const bloodCostToLevelUp = await ugGameContract.getFighterLevelUpBloodCost(
      props.level,
      1
    );
    const bloodCostToRankUp = await ugYakDenContract.getYakuzaRankUpBloodCost(
      props.rank,
      1
    );
    ...
    setBloodCostToRank(bloodCostToRankUp);

    //need to make sure amulet isnt expired so we can give accurate display
    const amuletDays =
      amulet?.lastLevelUpgradeTime + 7 * 86400 > Date.now() / 1000
        ? amulet?.level
        : 0;
    const timeLeftToLevelUp =
      props.lastLevelTime + (7 + amuletDays) * 86400 - Date.now() / 1000;
    const progressLevel =
      timeLeftToLevelUp > 0
        ? (timeLeftToLevelUp * 100) / ((7 + amuletDays) * 86400)
        : 0;
    ...
    const progressRaid =
      timeLeftToRaid > 0 ? (timeLeftToRaid * 100) / (7 * 86400) : 0;
      ...
      <CardActionArea onClick={clickHandler}>
        <CardContent align="center" sx={{ p: 0, color: "red" }}>
          {props.isFighter && (
            <Typography
              ...
              variant="body2"
              component="div"
              sx={{ fontFamily: "Roboto", fontSize: ".6rem" }}
            >
              {`#${props.id} `}
            </Typography>
          )}
          {props.isFighter && (
            <Typography
              ...
              variant="body2"
              component="div"
              sx={{ fontFamily: "Roboto", fontSize: ".6rem", color: "aqua" }}
            >
              {`  LEVEL: ${props?.level}`}
            </Typography>
          )}
          {props.isFighter && (
            <Typography
              sx={{ fontSize: ".6rem", color: "orange" }}
            >{`BR: ${props.brutality} Co: ${props.courage} Cu: ${props.cunning}`}</Typography>
          )}
        ...
        <CardContent align="center" sx={{ p: 0, color: "cyan" }}>
          {props.isFighter && (
            <Stack>
              <Typography
                sx={{ fontSize: ".6rem", color: "gold" }}
              >{`Level Up Cost: ${bloodCostToLevel}`}</Typography>
              <Box
                sx={{ display: "inline-flex", justifyContent: "space-evenly" }}
              >
                <Stack direction="row" spacing={2}>
                  <CircularProgressWithLabel
                    value={Math.ceil(progressForLevel)}
                    time={timeLeftForLevel}
                    style={{
                      color:
                        Math.ceil(progressForLevel) < 20 ? "red" : "chartreuse",
                    }}
                  />
                  <CircularProgressWithLabel
                    value={Math.ceil(progressForRaid)}
                    time={timeLeftForRaid}
                    style={{
                      color:
                        Math.ceil(progressForRaid) < 20 ? "red" : "chartreuse",
                    }}
                  />
                </Stack>
              </Box>
          ...
          {!props.isFighter && (
            <Typography
              ...
              variant="body2"
              component="div"
              sx={{ fontFamily: "Roboto", fontSize: ".75rem" }}
            >
              {`YAKUZA #${props.id} `}
            </Typography>
          )}
          {!props.isFighter && (
            ...
            >
              {`Rank Up Cost: ${Number(bloodCostToRank).toLocaleString()}`}
            </Typography>
          )}
        </CardContent>
      </CardActionArea>
