...
import {Box, Stack, Avatar, Typography, Button, Card,CardContent, CardMedia, CardActions } from '@mui/material/';
...
export default function RingMintBox() {
    ...
    const mintHandler = async (event) => {
        ...
        if(bloodBalance/(10**18) < 2000000){
            setError({
                ...
                message: 'You must acquire more $BLOOD to Mint a RING.',
            });
            return;
        }
        const isActive = await ugGameContract.RING_MINT_ACTIVE();
        if(!isActive){
            setError({
                title: 'RING MINT NOT ACTIVE!',
        ...
        const receipt = await signedContract.functions.mintRing() ;
        
        if(receipt.hash)  setError({
            title: `Minting a Ring!`,
    ...
    const getUpdates = async() => {
        const ringsMinted = await ugNftContract.ttlRings();
        setNumMinted(ringsMinted?.toString());
    }
  ...
  return (
    ...
    <Stack direction = "row" spacing = {5}>
        <Box  sx={{ maxWidth: 200}}>
        <Card  className='forge-bordr'  sx={{ maxWidth: 200, borderRadius: 5}}>
            ...
            alt="Mint Ring"
            height={180}
            sx={{ maxWidth: 200, maxHeight: 215}}
            />
             <CardContent >  
            <Typography variant="h6" align='center' sx={{fontFamily: 'Alegreya Sans SC', backgroundColor: 'black', color: 'red' }} >Mint Ring</Typography>
            <CardActions sx={{ backgroundColor: 'black', maxHeight: 50 }}>
               
                <Stack align="right" direction='row' sx={{mt: 2, backgroundColor: 'black', maxHeight: 50 }}>  
                    <Avatar className="forge2-bordr" sx={{ height: 50, width: 50 }} alt="Blood Token" src={bloodToken} />
                    
                    <Typography  align='center' sx={{fontFamily: 'Alegreya Sans SC',  color: 'aqua', fontSize: '1rem' }} > 2M Blood/Ring</Typography>
